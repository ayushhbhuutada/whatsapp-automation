    ...
    while (this.keepRunning) {
      ...
      for (let attempt = 0; attempt <= maxRetries; attempt++) {
        ...
        try {
          ...
          // Handle multiple attachments if defined
          if (contact.attachment_path && String(contact.attachment_path).trim()) {
            let rawPaths = [];
            const rawStr = String(contact.attachment_path).trim();

            if (rawStr.startsWith('[')) {
              try { rawPaths = JSON.parse(rawStr); } catch (e) {}
            }
            ...
            for (const rawPath of rawPaths) {
              ...
              if (fs.existsSync(fullPath)) {
                validFullPaths.push(fullPath);
              } else {
                await this.log(campaignId, contact.id, 'warning', `Attachment missing: ${path.basename(rawPath)}`);
              }
            }

            if (validFullPaths.length > 0) {
              const fileNames = validFullPaths.map(p => path.basename(p)).join(', ');
              await this.log(campaignId, contact.id, 'info', `Attaching ${validFullPaths.length} file(s): ${fileNames}...`);

              const hasMedia = validFullPaths.some(p => {
                const ext = path.extname(p).toLowerCase();
                return ['.png', '.jpg', '.jpeg', '.gif', '.webp', '.mp4', '.mov', '.avi', '.3gp', '.mkv', '.m4v'].includes(ext);
              });
              ...
              for (let menuAttempt = 0; menuAttempt < 3; menuAttempt++) {
                try {
                  ...
                  if (await this.page.isVisible(optionSelector).catch(() => false)) {
                    ...
                    await fileChooser.setFiles(validFullPaths);
                    fileUploaded = true;
                    break;
                  }
                } catch (e) {
              ...
              if (!fileUploaded) {
                ...
                if (fileInput) {
                  await fileInput.setInputFiles(validFullPaths);
                  fileUploaded = true;
                }
              }

              if (!fileUploaded) {
                throw new Error('Failed to open attach menu or upload multiple files.');
              }
